...
class ApiService
{
    ...
    public static function articles($param)
    {
        ...
        if (!empty($categoryIds)) {
            ...
            $field = !empty($param['field']) ? $param['field'] : 'post.*,min(post.category_id) as category_id';
            ...
            if (count($categoryIds) == 1) {
                $whereCategoryId = function (Query $query) use ($categoryIds) {
                    $query->where('post.category_id', $categoryIds[0]);
                };
            } else {
                $whereCategoryId = function (Query $query) use ($categoryIds) {
                    $query->where('post.category_id', 'in', $categoryIds);
                };
            }
        } else {
